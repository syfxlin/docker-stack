...
Optional arguments:
  -m <str>    Vhost generation mode. Possible values are:
              ...
              -m npm...: Generate http and https version (Nginx Proxy Manager)
...
def validate_args_req(name, docroot, proxy, mode, location):
    ...
    if proxy is not None:
        ...
        if port < 1 or port > 65535:
            print(
                "[ERR] Invalid reverse proxy port range: '%d', should between 1 and 65535" % (port),
                file=sys.stderr,
            )
            sys.exit(1)
    ...
    if mode is not None:
        if mode not in ("plain", "ssl", "both", "redir"):
            if mode != None and not mode.startswith("npm"):
                print(
                    "[ERR] Invalid -m mode string: '%s', should be: %s, %s, %s or %s"
...
def vhost_get_ssl_crt_path(config, server_name):
    ...
    prefix = to_str(config["vhost"]["name"]["prefix"])
    suffix = to_str(config["vhost"]["name"]["suffix"])
    if server_name.startswith("npm"):
      name = prefix + os.path.join(server_name, suffix, "fullchain.pem")
    else:
...
def vhost_get_ssl_key_path(config, server_name):
    ...
    suffix = to_str(config["vhost"]["name"]["suffix"])
    if server_name.startswith("npm"):
        name = prefix + os.path.join(server_name, suffix, "privkey.pem")
    else:
...
def vhost_get_server_status(config, template):
    ...
    return str_replace(status, {"__REGEX__": to_str(config["vhost"]["server_status"]["alias"])})
...
def get_vhost_ssl(config, tpl, docroot, proxy, location, server_name, default, ssl_path = None):
    """Get ssl vhost"""
    if ssl_path is None:
        ssl_path = server_name
    return str_replace(
        tpl["vhost"],
        {
            ...
            "__VHOST_RPROXY__": str_indent(vhost_get_vhost_rproxy(tpl, proxy, location), 4),
            "__REDIRECT__": "",
            "__SSL__": str_indent(vhost_get_vhost_ssl(config, tpl, ssl_path), 4),
            "__INDEX__": vhost_get_index(config),
...
def get_vhost(config, tpl, docroot, proxy, mode, location, server_name, default):
    ...
    if mode == "ssl":
        return get_vhost_ssl(config, tpl, docroot, proxy, location, server_name, default)
    if mode == "both":
        return get_vhost_ssl(
            config, tpl, docroot, proxy, location, server_name, default
        ) + get_vhost_plain(config, tpl, docroot, proxy, location, server_name, default)

    if mode == "redir":
        return get_vhost_ssl(
            config, tpl, docroot, proxy, location, server_name, default
        ) + get_vhost_redir(config, tpl, docroot, proxy, server_name, default)
    if mode != None and mode.startswith("npm"):
        return get_vhost_ssl(
            config, tpl, docroot, proxy, location, server_name, default, mode
        ) + get_vhost_plain(config, tpl, docroot, proxy, location, server_name, default)

    return get_vhost_plain(config, tpl, docroot, proxy, location, server_name, default)
...
def main(argv):
    ...
    if save:
        if not os.path.isdir(config["conf_dir"]):
            print("[ERR] output conf_dir does not exist:", config["conf_dir"], file=sys.stderr)
            sys.exit(1)
